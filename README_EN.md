...
`Chexson's AE Utils` is a utility add-on mod for Applied Energistics 2. The current `master` branch target stack is `Minecraft 1.20.1 + Forge 47.4.10 + AE2 15.4.5 + Java 17`. It adds a multi-slot ME emitter, processing pattern replacement rules, and ignore-missing autocrafting continuation for players and modpacks that need more precise ME network automation.

This README is for the `1.20.1 + Forge` branch. The `Minecraft 1.21.1 + NeoForge` migration branch uses the same documentation structure, but with different dependency and development-environment versions.
...
- Minecraft: `1.20.1`
- Forge: `47.4.10` or a compatible newer `47.x` build
- Applied Energistics 2: `15.4.5` or a compatible newer `15.4.x` build
- GuideME: `20.1.7`
...
This branch integrates with current Forge and AE2 APIs, menus, mixins, and runtime behavior. Re-run tests and in-game verification after updating AE2, Forge, or Minecraft.
...
The common config file is in the standard Forge `common` config location, usually:
...
2. Install Applied Energistics 2 `15.4.5+`.
...
- `test`: runs the currently configured regression tests.
...
- `gradle.properties`: target platform, dependency versions, and mod metadata.
...
- The current `master` branch targets `Minecraft 1.20.1 + Forge + AE2 15`.
...
- Minecraft, Forge, AE2, and Chexson's AE Utils versions.
