...
public final class BuildingGadgets2PatternEncoder {
    ...
    // ponytail: caller must provide explicit outputItem (no BG2 Registration.Template compile dep)
    public static BuildingGadgets2PatternEncodingResult encode(
        ...
        if (counts.isEmpty()) {
            LOGGER.warn("BG2 pattern encoding skipped template '{}' because no encodable materials were found", templateName);
        ...
        List<GenericStack> sortedInputs = counts.entrySet().stream()
                ...
                .toList();
        List<GenericStack> encodedInputs = sortedInputs.stream().limit(MAX_PROCESSING_INPUTS).toList();

        if (sortedInputs.size() > MAX_PROCESSING_INPUTS) {
            LOGGER.warn("BG2 pattern encoding truncated template '{}' from {} input types to AE2 limit {}",
                    templateName, sortedInputs.size(), MAX_PROCESSING_INPUTS);
        }
        ...
        if (outputStack == null) {
            ...
            return new BuildingGadgets2PatternEncodingResult(ItemStack.EMPTY, sortedInputs.size(), encodedInputs.size(), skippedStates);
        }

        ItemStack pattern = PatternDetailsHelper.encodeProcessingPattern(
                encodedInputs.toArray(new GenericStack[0]),
                new GenericStack[]{outputStack}
        );
        return new BuildingGadgets2PatternEncodingResult(pattern, sortedInputs.size(), encodedInputs.size(), skippedStates);
    }

    private static Map<AEKey, Long> collectStateRequirements(BlockState state, BuildingGadgets2StateDrops drops) {
        ...
        if (droppedStacks.isEmpty()) return Map.of();
    ...
    private static ItemStack createTemplateOutput(ItemLike outputItem, @Nullable String templateName) {
        ...
        if (templateName != null && !templateName.isBlank()) {
            output.setHoverName(Component.literal(templateName));
        }
        return output;
    }
