...
public final class MultiLevelEmitterUtils {
    ...
    private static final int BASE_VISIBLE_ROWS = 6;
    ...
    public static int calculateVisibleSlotCount(int configuredSlots, int capacity) {
        ...
        int baseVisible = Math.min(BASE_VISIBLE_ROWS, safeCapacity);
        if (configuredSlots < baseVisible) {
            return baseVisible;
        }
        return Math.min(safeCapacity, configuredSlots + 1);
    }
