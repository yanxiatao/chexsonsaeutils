 ...
 * （需求 5）：挖掘/拆除掉落的物品携带 CUSTOM_PATTERN_PAGES 组件，放置时
 * updateCustomBlockEntityTag 读回组件写入 BE。
 * <p>
 * 存储组件注册（阶段 5b 方式 B）：实现 {@link IStorageComponent} 与
 * {@link CustomPatternExpandableItem} 标记接口，使本物品可放入物质聚合器存储元件位、
 * 扩容 GUI 存储槽，并参与扩容（与框架版物品同一套共享逻辑）。
 */
public class CustomPatternProviderItem extends BlockItem implements IStorageComponent, CustomPatternExpandableItem {
    ...
    protected boolean updateCustomBlockEntityTag(
        ...
        int pages = stack.getOrDefault(ChexsonsaeutilsContent.CUSTOM_PATTERN_PAGES.get(), 1);
