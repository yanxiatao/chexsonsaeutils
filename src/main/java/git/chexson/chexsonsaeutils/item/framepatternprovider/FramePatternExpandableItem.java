 ...
 * <p>
 * 动机：扩容 GUI（{@link git.chexson.chexsonsaeutils.menu.framepatternupgrade.FramePatternUpgradeMenu}）
 ...
 * 扩容（FRAME_PATTERN_PAGES 组件语义），故抽象为标记接口，实现类即视为可扩容对象。
 * <p>
 * 泛化方案选择：相比「实现 IStorageComponent 且携带 FRAME_PATTERN_PAGES 组件」的
 ...
 * （可放入扩容 GUI 存储槽与物质聚合器存储元件位），并携带 FRAME_PATTERN_PAGES
 * 组件语义（getOrDefault 读取、set 写入，见
 * {@link git.chexson.chexsonsaeutils.registration.ChexsonsaeutilsContent#FRAME_PATTERN_PAGES}）。
 */
public interface FramePatternExpandableItem {
}
