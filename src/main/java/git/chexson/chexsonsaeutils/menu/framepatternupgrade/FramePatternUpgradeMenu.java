 ...
 * 外部插入经宿主库存回调）时自动检测——存储槽是可扩容样板供应器 + 输入槽有
 * 扩展物品 + 页数未达上限 → 循环消耗输入槽扩展物品，存储槽物品 FRAME_PATTERN_PAGES
 * 组件逐次 +1，直到页数达上限或输入槽空；条件不满足（如页数已满）不消耗。
 * 无确认按钮，页数经 @GuiSync 同步到客户端（提示文本显示）。
 * <p>
...
public class FramePatternUpgradeMenu extends AEBaseMenu {
    ...
    /**
     * 执行一次扩容：消耗 1 个扩展物品，存储槽物品页数 +1（达上限不消耗）。
     * <p>
     * I1 修复：重新读取实际页数后再次校验上限（Fail Fast），与 isExpandable 判定一致，
     * 防止条件竞态绕过（isExpandable 基于同步缓存，此处以物品组件实际值为准）。
     */
    private void expand() {
