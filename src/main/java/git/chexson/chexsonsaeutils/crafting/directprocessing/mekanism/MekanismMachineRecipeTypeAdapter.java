...
public final class MekanismMachineRecipeTypeAdapter implements MachineAdapterRegistry.MachineRecipeAdapter {
    ...
    public List<RecipeTypeCandidate> resolveCandidates(MachineIdentity identity) {
        if (!ModList.get().isLoaded(MekanismDirectProcessingSupport.MOD_ID) || identity == null) {
            return List.of();
        }
        ...
        RecipeType<?> recipeType = MekanismDirectProcessingSupport.resolveRecipeType(machineId);
        if (recipeType == null) {
            return List.of();
        }
        return List.of(new RecipeTypeCandidate(
                null,
                recipeType,
