 ...
 * IPatternDetails。本类把 FramePatternItem 接入该机制，使框架样板能被
 * PatternDetailsHelper.decodePattern 识别（供应器、样板终端等通用路径）。
 * 注册时机：Chexsonsaeutils 主类构造器（FMLCommonSetup enqueueWork）。
 */
public class FramePatternDecoder implements IPatternDetailsDecoder {
    public static final FramePatternDecoder INSTANCE = new FramePatternDecoder();

    private FramePatternDecoder() {
    }

    @Override
    public boolean isEncodedPattern(ItemStack stack) {
        return stack.getItem() instanceof FramePatternItem;
    }

    @Nullable
    @Override
    public IPatternDetails decodePattern(AEItemKey what, Level level) {
        if (level == null || what == null || !(what.getItem() instanceof FramePatternItem item)) {
            return null;
        }
